...
export const TeamChatView = () => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex bg-[#fbf9f6] text-slate-800 animate-fade-in select-none overflow-hidden">
      ...
      <aside className={`w-full md:w-80 lg:w-96 bg-white border-r border-slate-200 flex flex-col shrink-0 z-20 transition-all ${
        ...
        {/* Sidebar Header */}
        <div className="h-16 bg-slate-900 border-b border-orange-500/20 text-white px-4 flex items-center justify-between shadow-xs shrink-0">
      ...
      <div className={`flex-1 flex flex-col h-full ${
        ...
        {/* Chat Top Header */}
        <header className="h-16 bg-gradient-to-r from-orange-600 via-orange-500 to-amber-600 text-white px-3 md:px-5 flex items-center justify-between shadow-md shadow-orange-600/20 shrink-0 z-10">
          
          <div className="flex items-center gap-2">
            {/* Mobile Channel Switcher Button */}
            <button
              onClick={() => setShowChannelListMobile(true)}
              className="md:hidden flex items-center gap-1 p-1.5 -ml-1 rounded-full hover:bg-white/10 text-white transition-colors cursor-pointer"
              ...
              <div className="relative">
                {selectedRecipient ? (
                  <img
                    ...
                    className="w-10 h-10 rounded-full object-cover ring-2 ring-white/80 aspect-square"
                  />
                ) : (
                  <div className="w-10 h-10 rounded-full bg-gradient-to-r from-orange-700 to-amber-700 flex items-center justify-center font-bold text-white shadow-inner">
                    <Users className="w-5 h-5" />
                  </div>
                )}
                <span className="absolute bottom-0 right-0 w-3 h-3 bg-emerald-400 rounded-full border-2 border-orange-600" />
              </div>

              <div>
                <h2 className="text-sm font-bold tracking-tight leading-tight flex items-center gap-1.5 text-white">
                  <span>{selectedRecipient ? selectedRecipient.name : 'Founders Executive Circle'}</span>
                  {selectedRecipient && (
                ...
                <p className="text-[11px] text-orange-100/90 leading-tight truncate">
          ...
          <button
            ...
            className="px-3 py-1.5 rounded-xl bg-white/15 hover:bg-white/25 text-white text-xs font-semibold flex items-center gap-1.5 transition-colors cursor-pointer"
          >
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-6 space-y-3 bg-[#fbf9f6]">
          
          {/* Security Notice Pill */}
          <div className="flex justify-center mb-3">
            <div className="bg-orange-50 text-orange-950 border border-orange-200/80 rounded-xl px-4 py-1.5 text-[11px] text-center max-w-md shadow-xs">
          ...
          {activeMessages.map((msg) => {
            ...
            return (
              ...
              >
                <div
                  className={`max-w-[85%] md:max-w-[70%] rounded-2xl p-3 shadow-xs text-xs leading-relaxed space-y-1.5 relative break-words ${
                    isMe
        ...
        {/* Executive Input Bar */}
        <footer className="p-2 md:p-3 bg-slate-50 flex items-end gap-2 border-t border-slate-200 shrink-0">
          
          {/* Cloudinary File Attachment Trigger */}
          <button
            onClick={() => fileInputRef.current?.click()}
            title="Attach Image / Media"
            className="p-2.5 rounded-full bg-white hover:bg-orange-50 text-slate-600 hover:text-orange-600 border border-slate-200 transition-colors shadow-2xs cursor-pointer"
          >
          ...
          {/* Floating Pill Input */}
          <div className="flex-1 bg-white rounded-3xl border border-slate-200 shadow-2xs flex items-end px-3.5 py-1.5 gap-2 min-h-[44px] focus-within:border-orange-400 focus-within:ring-2 focus-within:ring-orange-500/20">
            <textarea
              value={inputText}
              ...
              rows={1}
              className="flex-1 text-xs text-slate-800 placeholder:text-slate-400 resize-none max-h-28 focus:outline-none py-1.5 leading-relaxed bg-transparent"
          ...
          <button
            ...
            className="w-11 h-11 rounded-full bg-gradient-to-r from-orange-600 to-amber-600 hover:from-orange-500 hover:to-amber-500 text-white flex items-center justify-center shadow-md shadow-orange-600/30 transition-all shrink-0 disabled:opacity-50 cursor-pointer"
            title="Send Message"
          >
            {isUploading ? (
