      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        {displayedProjects.map((project) => {
          ...
          return (
            <div
              key={project.id}
              className={`p-5 md:p-6 rounded-3xl transition-all duration-300 space-y-4 shadow-sm flex flex-col justify-between bg-white border border-slate-200/90 hover:shadow-xl ${
                isClient 
            ...
            >
              <div className="space-y-4">
                ...
                <div className="p-4 rounded-2xl border space-y-3 bg-slate-50 border-slate-200/80">
                  <div className="flex items-center justify-between text-xs font-semibold">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <button
                        ...
                        className={`text-[10px] font-bold px-2 py-0.5 rounded-lg border transition-all flex items-center gap-0.5 cursor-pointer shadow-2xs ${
                          isClient 
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-1">
                    {stages.map((stage) => {
                      ...
                      return (
                        <div
                          key={stage.id}
                          className={`p-2.5 rounded-xl border transition-all space-y-1.5 bg-white border-slate-200/90 shadow-2xs ${
                            isClient ? 'hover:border-orange-300' : 'hover:border-amber-300'
                          }`}
                        >
                          {/* Stage Title, Progress & Badge */}
                          <div className="flex items-center justify-between gap-1 text-[11px]">
                            <span className="font-bold truncate max-w-[130px] text-slate-800">
                              {stage.name}
                            </span>
                          ...
                          <div className="flex items-center justify-between text-[10px] pt-0.5">
                            <div className="flex items-center -space-x-1.5">
                              {stageFounders.length > 0 ? (
                                ...
                                <span className="text-[10px] truncate max-w-[100px] text-slate-400">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div className="p-2.5 rounded-2xl border border-slate-200/90 bg-slate-50 space-y-1">
                ...
                <div className="p-3.5 rounded-2xl border border-slate-200/90 bg-slate-50/70 space-y-2.5">
                  <div className="flex items-center justify-between">
                    <button
                      ...
                      className="flex items-center gap-1.5 text-xs font-bold text-slate-800 hover:text-orange-600 transition-colors cursor-pointer"
                    >
                    ...
                    <button
                      ...
                      className="text-[11px] font-bold flex items-center gap-1 px-2.5 py-1 rounded-xl text-orange-600 hover:text-orange-700 bg-white border border-orange-200 hover:border-orange-400 transition-all shadow-xs cursor-pointer"
                    >
                      <Plus className="w-3 h-3" />
                      <span>Add Task</span>
                    </button>
                    ...
                    <div className="space-y-1.5 pt-1 max-h-48 overflow-y-auto">
                      {projectTasks.length > 0 ? (
                        projectTasks.map(t => (
                          <div 
                            key={t.id} 
                            className="p-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 text-xs flex items-center justify-between gap-2 shadow-xs"
                          >
              ...
              {/* Bottom Actions: Credential Vault Trigger */}
              <div className="pt-3 border-t border-slate-100 flex items-center justify-between">
                {isClient ? (
                  ...
                  <button
                    onClick={() => onOpenVault(project)}
                    className="flex items-center gap-2 px-3.5 py-1.5 rounded-xl bg-amber-50 hover:bg-amber-100 text-amber-800 text-xs font-semibold border border-amber-200 transition-all shadow-xs cursor-pointer"
                  >
